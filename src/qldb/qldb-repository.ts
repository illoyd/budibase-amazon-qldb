...
export class QldbRepository {
    ...
    async insertInto(txn: TransactionExecutor, field: string, document: Record<string, unknown>, where: Record<string, unknown>) {
        const {fields, values} = this.splitFields(where);
        const results = (await this.do(txn, this.insertIntoQuery(field, fields), ...values, document))
        const id = results[0]
            .get(this.idFieldName)!
            .stringValue()!;
        return this.find(txn, id);
    }

    async update(txn: TransactionExecutor, document: Record<string, unknown>, where: Record<string, unknown>) {
    ...
    async upsert(txn: TransactionExecutor, document: Record<string, unknown>, where: Record<string, unknown>) {
        ...
        switch (existing.length) {
            ...
            case 1:
                return this.update(txn, document, where);
            default:
                throw Error(`Upsert matched multiple documents; searching for ${where}, found ${existing.length} documents`);
        }
    }
    ...
    async do(txn: TransactionExecutor, query: string, ...params: unknown[]): Promise<ion.dom.Value[]> {
    ...
    protected splitFields(obj: Record<string, unknown>) {
        ...
        return {
            fields: entries.map((entry) => entry[0]), values: entries.map((entry) => entry[1]),
        };
    }
    ...
    protected splitFieldsWithOperator(obj: Record<string, unknown>) {
        ...
        return {
            fields: entries.map((entry) => ({
                name: entry[0], operator: this.operatorForValue(entry[1]),
